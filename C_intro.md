...
## <a name="note">注意事項</a>
Javaと異なり、以下の点に注意が必要です。

### (1) 変数の初期値は初期化されない。（初期化が必須）
以下に変数を初期化した場合と初期化しなかった場合のコード例（Java, C）を示します。Javaの基本型は初期値を指定しなかった場合（以下では変数b）には所定の値（int型なら0）で必ず初期化されます。これに対し、C言語では初期化そのものが行われず、過去に保存されていた値がそのまま残った状態で変数が割り当てられます。
```Java
//Java
int a = 10;
int b; // 0 (自動で初期値が設定される)
...
int a = 10;
int b; // なにか変な値
printf("%d", b); // 何か変な値が出力される
```

### (2) 配列のインデックスに注意。
以下に配列に対してループ文を書いたコード例（Java, C）を示します。Javaでは予め用意した要素数を超える部分（以下だとa[3]やa[4]など）に対して参照することそのものが実行エラーとなります。これに対し、C言語では警告(warnings)は出ますが、実行そのものはできてしまいます。
```Java
//Java
int a[3] = {10, 20, 30};
System.out.println("a[0] = ", a[0]); //10
System.out.println("a[3] = ", a[3]); //IndexOutOfErrror（実行そのものができない）
```
...
int a[3] = {10, 20, 30};
printf("a[0] = ", a[0]); //10
printf("a[3] = ", a[3]); //何か変な値が出力される（実行できてしまう）
```
